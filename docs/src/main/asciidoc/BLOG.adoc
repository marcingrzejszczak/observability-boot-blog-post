...
example of such a key value (for example, `/foo/user1234`, `/foo/user2345`, and so on). *Low cardinality* means that a key value will  have a bounded number of possible values. A *templated* HTTP URL (such as `/foo/{userId}`) is a good example of such a key value.
...
Now we need to add some configuration. We set up `actuator` and `metrics` to publish percentiles histograms, and we redefine the logging pattern to include the trace and span identifiers.
...
We want to have some detailed observation of the `MyService#foo` method. Thanks to having added AOP support, we can use the `@Observed` annotation. To do so, we can register a `ObservedAspect` bean.
...
With metrics and tracing on the classpath, having this annotation leads to the creation of a `timer`, a `long task timer`, and a `span`. The timer would be named `foo.metric`, the long task timer would be named `foo.metric.active`, and the span would be named `my-contextual-name`.
...
*** The other application used in this demo uses another tracer library to show an interop between tracers.
...
Now we need to add some configuration. We add almost identical configuration as we did on the server side. However, we also add `management.tracing.sampling.probability=1.0` to ensure that all spans are always sent to the latency analysis server.
