...
** For *Latency Visualization*, we need to send the finished spans in some format to a server. In our case, we produce an OTLP-compliant span. To achieve that, we need to add the `io.zipkin.contrib.otel:brave-encoder-otel` dependency.
// TODO: Add info about additional configuration (the ByteSender)
* *Logs*
